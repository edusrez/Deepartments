...
test('fb-1289 — the verdict is on the RESOLVED DESTINATION (kernel order): a symlinked component + `..` is decided PHYSICALLY, never lexically', async () => {
  const base = await realpath(await mkdtemp(path.join(tmpdir(), 'fb1289-link-')))
  const root = path.join(base, 'scoped')          // the ONLY allowed root
  const ws = path.join(root, 'dept-ws')           // the guard cwd
  const outside = path.join(base, 'outside')      // OUT-of-root directory
  const shared = path.join(root, 'shared')        // IN-root directory
  await mkdir(ws, { recursive: true })
  await mkdir(outside, { recursive: true })
  await mkdir(shared, { recursive: true })
  await writeFile(path.join(base, 'secret.txt'), 'secret\n')
  await writeFile(path.join(shared, 'in.txt'), 'in\n')
  // A symlink INSIDE the root pointing OUTSIDE it — the shape that makes the
  // lexical and the physical resolution DIVERGE.
  await symlink(outside, path.join(root, 'link'))
  // A symlink inside the root pointing INSIDE it (the allow-direction leg).
  await symlink(shared, path.join(root, 'alias'))
  const roots = [root]

  // The divergence, stated as a fact of the fixture: lexically,
  // `../link/../secret.txt` looks like `root/secret.txt` (IN root → would
  // ALLOW); the kernel resolves `root/link` to `base/outside` FIRST, so the
  // following `..` is `base` and the destination is `base/secret.txt` (OUT of
  // root → DENY).
  const relWord = '../link/../secret.txt'
  const lexical = path.resolve(ws, relWord)
  assert.equal(lexical, path.join(root, 'secret.txt'), 'the LEXICAL resolution of ../link/../secret.txt lands IN root (the naive reading)')
  assert.equal(await realpath(path.join(root, 'link')), outside, 'the symlink really points OUT of the root')

  // ---- DENY leg: same destination, two forms → DENY/DENY.
  const denyPair = [verdict(`cat ${path.join(base, 'secret.txt')}`, ws, roots), verdict(`cat ${relWord}`, ws, roots)]
  assert.deepEqual(denyPair, ['DENY', 'DENY'], `the symlinked traversal to an out-of-root destination is DENIED in BOTH forms — got ${JSON.stringify(denyPair)} (a lexical-only fix would ALLOW the relative leg)`)

  // ---- ALLOW leg (no over-blocking): a symlink that stays INSIDE the root —
  // `../alias/../shared/in.txt` → alias→root/shared, `..`→root, so the
  // destination is the SAME in-root file as the absolute form.
  const allowPair = [verdict(`cat ${path.join(shared, 'in.txt')}`, ws, roots), verdict('cat ../alias/../shared/in.txt', ws, roots)]
  assert.deepEqual(allowPair, ['ALLOW', 'ALLOW'], `an in-root symlinked traversal stays ALLOWED in BOTH forms — got ${JSON.stringify(allowPair)}`)
})
