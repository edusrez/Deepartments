...
test('boot-factory: the BOOT ZONE (config source + registry + catalog + lifecycle tools + presence + boot hooks) was hoisted VERBATIM into the orchestration factory (the artifact + the movement lock)', () => {
  ...
  // zone has no module-position-dependent expression; md5 stamp 82761e5d…).
  {
    ...
    assert.equal(zoneText.split('\n').length - 1, 741, 'the embedded boot zone is exactly 741 content LOCs (677 pre-lane-0.1.2 + the 2 rc.1 surface-comment lines on the headProgress tracker + the VALLE 09-07 HEAD-TOOLING fb-216/223 memo-validation lines + the 2 fb-473/05cb3cb dept_sleep `reason` optional-parameter lines: the description phrase was added IN PLACE on the existing line (0 LOC) and the `parameters: {},` → `parameters: { reason: { type: \'string\', description: … } },` split added the 2 content LOCs)')
    const md5 = createHash('md5').update(zoneText, 'utf8').digest('hex')
    assert.equal(md5, 'bbba9efd92c7edb282e2ed66fc8b79ea', 'the embedded boot zone is byte-identical to HEAD applyInvoke 2345-3021 with the LANE 0.1.2 rc.1 session-surface comment, the VALLE 09-07 HEAD-TOOLING memo-validation move (memoWriteArgsViolations + the json-typed parameters + the execute validator) AND the fb-473/05cb3cb dept_sleep reason-parameter composition — RE-FROZE: 741 LOCs, md5 bbba9efd… (the md5 moved WITH the LOC pin: both are the SAME movement identity and are re-frozen together; R-tooling DX, the batch-drain/delivery semantics untouched)')
