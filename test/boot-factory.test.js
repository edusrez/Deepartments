...
test('boot-factory: the BOOT ZONE (config source + registry + catalog + lifecycle tools + presence + boot hooks) was hoisted VERBATIM into the orchestration factory (the artifact + the movement lock)', () => {
  ...
  // zone has no module-position-dependent expression; md5 stamp 82761e5d…).
  {
    ...
    assert.equal(zoneText.split('\n').length - 1, 751, 'the embedded boot zone is exactly 751 content LOCs (677 pre-lane-0.1.2 + the 2 rc.1 surface-comment lines on the headProgress tracker + the VALLE 09-07 HEAD-TOOLING fb-216/223 memo-validation lines + the 2 fb-473/05cb3cb dept_sleep `reason` optional-parameter lines: the description phrase was added IN PLACE on the existing line (0 LOC) and the `parameters: {},` → `parameters: { reason: { type: \'string\', description: … } },` split added the 2 content LOCs + the 10 fb-946/9184241 HOST-MUDO lines: the `onMuteHostAnomaly` boot hook (boot.ts:800) that CONSUMES the durable registry anomaly the fix exists to emit — a REAL behavioural addition to the zone, not a formatting drift, so the pin moves WITH the behaviour)')
    const md5 = createHash('md5').update(zoneText, 'utf8').digest('hex')
    assert.equal(md5, '2438d85da5846190e53261607986a59e', 'the embedded boot zone is byte-identical to HEAD applyInvoke 2345-3021 with the LANE 0.1.2 rc.1 session-surface comment, the VALLE 09-07 HEAD-TOOLING memo-validation move (memoWriteArgsViolations + the json-typed parameters + the execute validator), the fb-473/05cb3cb dept_sleep reason-parameter composition AND the fb-946/9184241 HOST-MUDO boot hook — RE-FROZE: 751 LOCs, md5 2438d85d… (the md5 moves WITH the LOC pin: both are the SAME movement identity and are re-frozen together; the line count moved 741 → 751 because the `onMuteHostAnomaly` hook is a REAL behavioural addition inside the zone, not drift)')
