...
test('m-2333 (b) — the HALT boundary: 1 usable key with monthly available < 10% → HALT (dispatch blocked + critical finding); 1 usable key with weekly available < 20% → HALT', async () => {
  await withTempStateDir(async (stateDir) => {
    ...
    assert.match(mBlock.reason, /^pool: HALT — 1 usable key oc-6 \(monthly available 5% < 10%\) — NO new dispatches until ≥2 usable keys or new keys are added$/, 'the monthly-leg HALT reason names the key + the 5% monthly available + the threshold, and ONLY the arm that fires (fb-632: the old literal also asserted the weekly arm, which is FALSE here)')
    ...
    assert.match(wBlock.reason, /^pool: HALT — 1 usable key oc-6 \(weekly available 15% < 20%\) — NO new dispatches until ≥2 usable keys or new keys are added$/, 'the weekly-leg HALT reason names the key + the 15% weekly available + the threshold, and ONLY the arm that fires (fb-632)')
...
test('O1 (g) — C/D records ABSENT or NULL → 0 regression: the healthy pooler shapes (no billingDown/halted fields — legacy — OR explicit billingDown:null / halted:null — the current toState shape) pass through exactly like before; the m-2333 HALT and the CERTAIN 0-usable outage are untouched when no durable record rides the snapshot', async () => {
  await withTempStateDir(async (stateDir) => {
    ...
    assert.match(haltBlock.reason, /^pool: HALT — 1 usable key oc-6 \(weekly available 15% < 20%\) — NO new dispatches until ≥2 usable keys or new keys are added$/, 'the m-2333 HALT reason is unchanged in SUBSTANCE when no durable record rides the snapshot (the wording of the «or» arms was corrected by fb-632; the verdict, the key and the threshold are identical)')
