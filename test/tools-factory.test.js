...
test('tools-factory: the TOOLS ZONE CUTS 1+2+3 were hoisted VERBATIM into the orchestration factory (the artifact + the movement lock)', () => {
  ...
  // fe319e0f4f0c145081e1e46bf0e9d7b3 → 5b548545a2971a76a79fab0dbff8411d.
  {
    ...
    // Zone md5 RE-FROZE R10 (R2 lane, 2026-09-04 — fb-42/25 rotation-model
    // probe + fb-25 GAP-2): TWO INTENTIONAL in-span changes in the
    // dept_head_rotate tool — (1) the ROTATION-MODEL PROBE pre-check runs in
    // the execute BEFORE the dispose/archive steps (an unknown-model rotation
    // aborts with the old head untouched — fb-42 class), and (2) the reason
    // cross-check passes the fb-50 completion-reserve calibration (the «N% de
    ...
    // Zone md5 RE-FROZE (2026-09-17, lane fb-1879 — the `dept_feedback` OUTPUT
    // SCHEMA vs the fb-1874 candidate emission / run token 38362679): ONE
    // INTENTIONAL in-span change, and it is ADDITIVE — the declared
    // `feedbackDedupeCandidateSchema` gained the 6 fields the emission was
    // ALREADY producing (`admissible`, `relation`, `relation_because` +
    // the optional `destination`/`linked_from`/`resolved_from`), so the harness
    // output validator stops rejecting a value the tool ALREADY wrote (with ≥1
    // duplicate candidate every emit answered
    // `returned invalid output: "value.candidates[0].admissible" is not a
    // declared property (additionalProperties: false)` while the record WAS
    // durable — the emitter then believed its record was lost). NOTHING ELSE
    // MOVED: no branch, no parameter, no render, no verdict, and the 6
    // pre-existing properties keep their exact declarations. md5
    // 6d722642f8284422ea666bab58e864e7 → 8ffce0e12fc65ade2050e54a5d084932;
    // M E A S U R E D (not invented) with the SAME extraction this block performs
    // (banner → host-plane effect close), from the repo root:
    //   node -e "const{createHash}=require('node:crypto');const fs=require('node:fs');const src=fs.readFileSync('packages/dshd-orchestration/src/tools.ts','utf8');const B='  // --- messaging bus TOOL DEFINITIONS (ONE body per tool; registered in the';const C=\"  }, 'deepartments: host-plane tools')\";const f=src.indexOf(B),l=src.indexOf(C);console.log(createHash('md5').update(src.slice(f,l+C.length)+'\n','utf8').digest('hex'))"
    // (an INDEPENDENT python extraction of the same span returned the same
    // value — the measurement is reproducible by two instruments), and
    // byte-synced with scripts/zone-md5-manifest.json cut4-tools-zone and the
    // FROZEN_MD5 literal of test/cut4-zone-lock.test.js in the SAME change (the
    // header rule: ALL THREE carriers of the movement identity travel
    // together — a re-freeze that updates only two leaves the guard lying).
    // Zone md5 RE-FROZE (2026-09-21, lane fb-2181 — the non-QH fold is
    // UNREACHABLE because the information arrives AFTER the act / run token
    // 78ed6bf5): ONE INTENTIONAL in-span change, and it is PURELY ADDITIVE — a
    // NEW read-only tool `dept_feedback_candidates` (the PRE-WRITE duplicate
    // query: the SAME `dedupeCandidates` search the create runs, with ZERO
    // writes) + its registration in the UNIVERSAL `feedbackEmitTools` array (so
    // a WORKER — the emitter without terminal authority — sees it) + the 10th
    // global host-plane registration `globalFeedbackCandidates` + itself in the
    // disposal closure + one sentence added to the CREATE tool description
    // pointing at the pre-write query. NOTHING ELSE MOVED: no existing tool
    // body, no parameter, no render, no branch of the state machine, and the
    // TERMINAL transition stays QH-only (spec §4 — the fix is TEMPORAL: the
    // canonical is now known BEFORE the write, so the fold lands in the CREATE,
    // which was ALREADY ACL-free for any emitter). md5
    // 8ffce0e12fc65ade2050e54a5d084932 → 5dc4d300ed610e2040733ef57fa91da9;
    // M E A S U R E D (not invented) with the SAME extraction this block performs
    // (banner → host-plane effect close), from the repo root:
    //   node -e "const{createHash}=require('node:crypto');const fs=require('node:fs');const src=fs.readFileSync('packages/dshd-orchestration/src/tools.ts','utf8');const B='  // --- messaging bus TOOL DEFINITIONS (ONE body per tool; registered in the';const C=\"  }, 'deepartments: host-plane tools')\";const f=src.indexOf(B),l=src.indexOf(C);console.log(createHash('md5').update(src.slice(f,l+C.length)+'\n','utf8').digest('hex'))"
    // byte-synced with scripts/zone-md5-manifest.json cut4-tools-zone and the
    // FROZEN_MD5 literal of test/cut4-zone-lock.test.js in the SAME change.
    // Zone md5 RE-FROZE (2026-09-22, lane fb-2112 — the `dept_feedback_update`
    // CONFIRMED NO-OP / run token 3f092289): ONE INTENTIONAL in-span change, and
    // it is the SYMMETRIC twin of the fb-775 lane above. fb-775 closed «an
    // UNDECLARED key» (the whitelist) and left open «NO key at all»: `{ id }`
    // alone passed the closed-set check (`id` IS expected), built the EMPTY
    // `input`, and `store.update` returned the record with `status:"settled"` —
    // SUCCESS for a transition that never happened, with no `cause` to filter, so
    // no TOOL+CAUSE+ts census can see it. The change: the ONE violation the two
    // doors share (`no update field applied: the call would apply ZERO
    // transition` + the declared write fields) + the WRITE-field test inside the
    // SAME `feedbackUpdateArgsViolations` (same family, same ONE message, and the
    // fb-775 ordering preserved) + the runtime anchor `Object.keys(input).length
    // === 0` immediately before `store.update` (which also catches the `estado:
    // ''` empty-slot shape door 1 cannot see) + the tool-description sentence.
    // NOTHING ELSE MOVED: no transition semantics, no authority guard, and the
    // `!== undefined` constructor is untouched, so every id + ONE declared field
    // call (incl. `related: []`, `escalado: false`, `notas_qh: ''`) keeps its
    // EXACT behaviour. md5 5dc4d300ed610e2040733ef57fa91da9 →
    // f7ed6986b8c89fa625910e3566b51fbd;
    // M E A S U R E D (not invented) with the SAME extraction this block performs
    // (banner → host-plane effect close), from the repo root:
    //   node -e "const{createHash}=require('node:crypto');const fs=require('node:fs');const src=fs.readFileSync('packages/dshd-orchestration/src/tools.ts','utf8');const B='  // --- messaging bus TOOL DEFINITIONS (ONE body per tool; registered in the';const C=\"  }, 'deepartments: host-plane tools')\";const f=src.indexOf(B),l=src.indexOf(C);console.log(createHash('md5').update(src.slice(f,l+C.length)+'\n','utf8').digest('hex'))"
    // byte-synced with scripts/zone-md5-manifest.json cut4-tools-zone and the
    // FROZEN_MD5 literal of test/cut4-zone-lock.test.js in the SAME change.
    assert.equal(md5, 'f7ed6986b8c89fa625910e3566b51fbd', 'the embedded CUT4 zone matches the LANE 0.2.3b re-freeze + the session-surface reads + the LANE ② sweep/O1 additions + the R4 DUAL-read + the R6 getSessionEvents-collapse + the R9 WAKE-SEAM send_message prepared-class enrichment + the R10 R2 probe/pre-check additions + the R8-RACE liveness-race liveStatus + settle-wait additions + the R11 WAKE-SEAM P1-EXT managerId row + the R12 sweep-dormancy host-only recipientDormantForRedeliver wrapper + 2 re-delivery injections + the VALLE 09-07 BATCH-DRAIN batchEligible flag + prepared-class envelope + the VALLE 09-07 HEAD-TOOLING fb-216/223 memo-validator move + O4 legacy title + fb-220 rotate diagnostics + fb-209a dept_repo_state registration + the VALLE 09-07 FB-198 T1 failedGround observer + prepared (wake-failed) / failed:<ground> envelope + id-truth assertion + the LOOP FASE 1 feedback dedupe/duplicado/bridge-adjacent tool span + the fb-775 dept_feedback_update CLOSED-SET org validator + the LANE DEL SELLO reasonProvenance declared-origin contract (parameter schema + call-site threading + the ADDITIVE reasonReferenceScope/reasonVerifiedToken/declaredReferenceSessionId outputs) + the fb-831 dept_who declared-field render tail (the REQUIRED sessionId + departmentId/role/jobId, absent keys declared as `-`) + the 2026-09-17 BLOCKER lane (the `dept_head_rotate` RUNNING refusal error string declaring the OPT-IN LEVER — text only) + the 2026-09-17 fb-1879 lane (the ADDITIVE 6-field extension of feedbackDedupeCandidateSchema — admissible/relation/relation_because + the optional destination/linked_from/resolved_from) + the 2026-09-21 fb-2181 lane (the ADDITIVE read-only `dept_feedback_candidates` PRE-WRITE query tool + its universal own-layer/host-plane registration + the create-description pointer; the terminal transition stays QH-only) + the 2026-09-22 fb-2112 lane (the id-only CONFIRMED NO-OP cure: the shared no-update-field violation + the WRITE-field test in the SAME validator + the runtime anchor on the constructed `input` before `store.update` + the description sentence; the transition semantics and the `!== undefined` falsy distinction untouched) (md5 f7ed6986b8c89fa625910e3566b51fbd)')
  }
...
test('tools-factory (composed boot): the registry wiring is intact — the runner gates + the late-seam accessors exist, workerSetup + the CUT3 retire/workspace closures + the CUT4 tool arrays/driver are factory-locals (no late seams), the 4 BASELINE deps holders are FILLED by the factory (the dead binder is gone), NO deepartments.tools provided (P1)', async () => {
  ...
  try {
    ...
    try {
      ...
      for (const seam of ['busMemberIdFor', 'feedbackStoreReady', 'resolveQualityHeadEntry', 'feedbackForwarderFor', 'feedbackDeliveryOptions', 'busProfileFor', 'aclDenyGround', 'resolveBusCatalogRoute', 'delivery', 'isDormantRecipient', 'recipientMaterialized', 'recipientRunningLive', 'busEnsureHostForCaller', 'assertBusFanOut', 'busDeliverToPost', 'busDeliverToHost', 'resolveBusChild', 'deliverBusChild', 'freshMintHead', 'enqueueHostWake', 'reassertPostToolset']) {
      ...
      // The invocation late object carries EXACTLY the 24 TDZ-safe seams (the
      ...
      // 20th CUT4 seam, d9cc05ce; the VALLE 09-09 fb-300/fb-301
      // toolset-reassertion seam reassertPostToolset adds the 24th — the boot
      // heal's re-derivation action, a NEW late delivery-surface seam, 0 new
      // bundle exports).
      ...
      assert.equal(getterCount, 24, `the TOOLS invocation late object carries exactly 24 getters (found ${getterCount})`)
