 ...
 * (HITO 3 DECOUPLING, brief step 5 — SUB-BATCH 3 of 4: the workspace +
 * ensureHead + retire/archive + boot-check/reconcile closures (retirePost /
 ...
 * ensureHead / makeEntry / ensureAllHeads / headEventCount / markHeadProgress /
 * isHeadStuck / runPresetAudit / runInterruptedPostReconciliation /
 * runProviderAdapterBootCheck / runReasoningContentBootAssert /
 * runDurableRegistryReconciliation / GHOST_SUSPECT_DEFAULT_* /
 * runGhostSuspectReconcile / runHalfSleptHeadReconcile /
 * runRetiredWorkerResidueReconcile + the BOOT WIRING), 1273 LOCs of
 * `applyInvoke`, src/invoke.ts 4099-5371; SUB-BATCH 2 was the role/persona +
 * architecture prompt sections + the post setup closures (postSetup /
 * headSetup / workerSetup) + the head-dispose + retire-helper closures, 612
 * LOCs, src/invoke.ts 4061-4672 — pre-SB1 4900-5511; SUB-BATCH 1 was the
 * dept_exec/zstd runners + the agent toolset REGISTRY (installHeadBoardTools),
 * 922 LOCs, src/invoke.ts 3977-4898).
 *
 * MOVEMENT-ONLY. The THIRD cut of the tools zone of `applyInvoke` is hoisted
 * VERBATIM into this factory, and `applyInvoke` invokes it via
 ...
 * `deps`. The file is CUMULATIVE: sub-batch 4 of the tools zone (bus/feedback
 * defs + globals) grows this factory and its surface.
 *
...
import type { HealthSessionEvent, InterruptedPostInput } from '../health.js'
import {
...
import { qualityInspectDecision, QUALITY_INSPECT_ENV_VAR } from '../quality.js'
import type { QualityInspectDirectiveSurface } from '../quality.js'

import { isArchivedSession } from '../session-rotation.js'
import type { WorkspaceRegistryLike } from '../session-rotation.js'
import { headPresetIdFor } from '../../head-presets.js'
import {
  mintWorkerSessionId,
  HEAD_SESSION_PREFIX,
  headSessionId,
...
import type { PostEntry, RegistryStore, HostEntry } from '../registry.js'
...
export interface ToolsFactoryDeps {
  ...
  late: {
     ...
     * worker-retire QD directive emitter retirePost consumes at retire-time —
     * built at the DELIVERY factory position, AFTER this factory). */
    maybeEmitQualityInspectDirective: (surface: QualityInspectDirectiveSurface) => Promise<void>
    /** The DeliverySurface's boot re-delivery driver (the embedded boot wiring
     * calls `.run()` after the registries load — built at the delivery factory
     * position, AFTER this factory). */
    redeliverPendingDeliveries: { run: () => Promise<void> }
...
export function createToolsOrchestration(ctx: Context, deps: ToolsFactoryDeps): ToolsSurface {
  ...
  // execute, the retire seam, or the boot wiring actually fires — post-boot —
  // so the apply-scope TDZ is never entered). The CALL seams are thunk arrows
  // of the exact signatures; the store seam is a THENABLE delegating to the
  // surface's boot-opened promise (the zone awaits `messagesStoreReady` as a
  // value); the three for..of tool arrays are delegating ITERABLES (the zone
  ...
  // NOTE (SUB-BATCH 3): `resolveDepartmentWorkspaceCwd` / `resolveWorkspaceRootPath`
  ...
  // workerSetup resolve the SAME local consts at call time).
  ...
  const redeliverPendingDeliveries: ToolsFactoryDeps['late']['redeliverPendingDeliveries'] = { run: () => late.redeliverPendingDeliveries.run() }
