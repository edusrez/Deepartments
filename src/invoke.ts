...
export function applyInvoke(ctx: Context, config: Config) {
  ...
  const toolsSurface: ToolsSurface = (ctx.get('deepartments.tools') as ToolsSurface | undefined) ?? createToolsOrchestration(ctx, {
    ...
    // and are dereferenced ONLY when a registration / tool execute / the boot
    // wiring fires (post-boot — never here, so the TDZ of these later consts is
    // never entered). NOTE (SUB-BATCH 2): `workerSetup` is NOT a late seam
    // anymore — the CUT2 zone (installed in this factory) now DEFINES it.
    // NOTE (SUB-BATCH 3): `resolveDepartmentWorkspaceCwd` /
    // `resolveWorkspaceRootPath` / `retirePost` / `archiveWorkerSession` are NOT
    // late seams anymore either — the CUT3 zone (installed in this factory) now
    // DEFINES them; the 2 NEW late seams are the delivery factory's
    // `maybeEmitQualityInspectDirective` (retirePost's QD emitter) +
    // `redeliverPendingDeliveries` (the boot wiring's re-delivery driver).
    late: {
      get maybeEmitQualityInspectDirective() { return maybeEmitQualityInspectDirective },
      get redeliverPendingDeliveries() { return redeliverPendingDeliveries },
      get deliverBusRecord() { return deliverySurface.deliverBusRecord },
      get messagesStoreReady() { return deliverySurface.messagesStoreReady },
      get busTools() { return busTools },
      get feedbackEmitTools() { return feedbackEmitTools },
