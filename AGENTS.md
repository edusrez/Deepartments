...
1. `pnpm build` — **this IS the deploy-ladder gate** (`node scripts/check-root-build.mjs`): it verifies/regenerates every compiled `lib/` the runtime resolves (the root — `main: lib/index.js` — and each `packages/*/lib` behind the `node_modules/dshd-*` symlinks), then runs tsc NodeNext (src → lib) over the ROOT `src/`, no type errors, and finally RE-MEASURES before it prints PASS. The bare tsc keeps its own name: `pnpm build:tsc`. Read-only detection: `pnpm build:check` (never writes; exit 1 on any stale/missing artifact). WHY it is wired as step 1: `lib/` is gitignored (`.gitignore:7`), so the artifact does NOT travel in the commit and the machine that boots must regenerate it — with the gate documented but NOT wired (ladder §9, doc-only, no CI in this repo) a fully green ladder still loaded PRE-FIX code on the first restart (fb-2673 → fb-2230, fb-1855; ladder §9.1).
...
4. Real headless smoke in the twin profile — **pin the CWD to `/`** (the daemon's `WorkingDirectory`): `cd / && DSH_HOME=/opt/dsh/.dsh-dev dsh --profile deepartments-dev-headless "<prompt>"` (the GUI profile `deepartments-dev` rejects CLI prompt arguments). The `stateDir` is RELATIVE (`.deepartments`, `packages/dshd-core/cordis.patch.yml:43`) and resolves against the invocation CWD — running the smoke from `/root` or the repo cwd REGENERATES the parallel minimal store `/root/.deepartments` (fb-134 class, see `docs/STORES-MAP.md` §2.2); cwd=/ keeps the smoke on the SAME `/.deepartments` the daemon uses.
