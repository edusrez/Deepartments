 ...
 */
// LANE 0.2.2 (gap 2) — dshd-orchestration: the MOVED factory keeps its
// @deepseek-ai + node imports, but the bundle-local modules CANNOT be imported
// from the package (a require cycle: dsh-deepartments depends on this package).
// The org config TYPE mirrors live in ./org-types.js; the bundle-local VALUE
// imports (computeDeptWhoState) are now DEPS the bundle passes by reference
// (the "functions, never imports" rule — the same way readPresenceStateFile
...
import { ASISTENTE_SESSION_TITLE } from 'dshd-core'
...
export interface BootFactoryDeps {
  ...
  /** LANE 0.2.2 (gap 2) — the C1/C3 catalog state aggregate helper
   * (src/agents.ts — bundle-local, injected by reference like the other
   * module-scope pure helpers; the package cannot import it without a cycle). */
  computeDeptWhoState: (input: { retired: boolean; sleeping: boolean; live: boolean; running: boolean }) => DeptWhoState
