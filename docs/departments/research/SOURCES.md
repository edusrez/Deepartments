...
| `mimo.xiaomi.com` | **200-INSERVIBLE (shell JS)** | HTTP 200 y el cuerpo es SOLO el `<title>` («MiMo-V2.6 \| Xiaomi»); SPA Rspress v1.46.2 ⇒ el contenido NO llega a `web_fetch` (round 09-23, `daily-ai-news-21`; instancia **fb-2625** de **fb-843**, **reproducida por el QD** el mismo día con su propio `web_fetch`) | **one attempt max**; los hechos los llevan las **surfaces de anuncio fechadas** (post de Xiaomi MiMo en X + `mimo.mi.com/docs/en-US/updates/model`) y **secundarias fechadas** — NO es una primaria legible por fetch |
| `r.jina.ai/<url>` (proxy de render) | **NO UNIVERSAL — verificar POR HOST** | POSITIVA en `docs.hetzner.com` (cuerpo completo) y `servicenow.github.io/eva`; **NEGATIVA en `mimo.xiaomi.com`: HTTP 200 con `data.content:""` — PERO trae `title`, `description` (con texto real) y `canonical`** ⇒ **modo «METADATA-COMO-CUERPO», no «vacío»** (**fb-2625** → **fb-843**, abierto; verificado por RD **y por el QD**; relacionada con `fb-1168` — abierta; ⚠️ `fb-211` = **resuelto** y `fb-739` = **duplicado**→`fb-24`: **TERMINALES, no usar como hogar**) | **workaround POR HOST, NUNCA árbitro.** Predicado robusto: no «¿vino vacío?» sino **«¿este texto PASÓ por el render de la página, o es su metadata?»** — el `description` se lee como sustancia y es la tarjeta social. ⚠️ Su `warning` nombra dos salidas (**iframe processing** / **caching opt-out**) que se piden por **cabecera HTTP**, y `web_fetch` expone **sólo `url`** ⇒ **inalcanzables desde el llamante**: se sostiene que falla **en su forma por defecto** y que el remedio está gateado; **NO se publica «no sana nunca»** (negativo sin control) — discriminador **PENDIENTE DE VERIFICAR** |
...
| `mimo.xiaomi.com` (SPA Rspress) | 200 con cuerpo = **solo el `<title>`** (shell JS, no truncado) | surfaces de anuncio **fechadas** + secundarias fechadas; **`r.jina.ai` NO ayuda EN SU FORMA POR DEFECTO** (200 con `content:""` **+ metadata**: ver la fila del proxy) — su remedio va por cabecera ⇒ **no alcanzable desde `web_fetch`**; **NO concluir «el proxy no sana nunca»** (pendiente de verificar) |
