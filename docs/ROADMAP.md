...
- **2026-08-19** — **`muse-spark-1.2-contributor` enabled** (dev home `/opt/dsh/.dsh-dev/settings.yaml` only, per owner decision): new LLM provider route `opencode-go-responses` (api `openai-responses`, baseURL `https://opencode.ai/zen/go/v1`, model `muse-spark-1.2-contributor`, 1M context, maxTokens 131072). Research: `.dsh/reports/researcher/2026-08-19-opencode-go-muse-spark.md`; route analysis: `.dsh/reports/explore-deep/2026-08-19-opencode-go-muse-route.md`. Operational notes: Contributor tier = Meta trains on your prompts/completions (Not ZDR) → route only non-sensitive work; muse responds only after the OpenCode Go console **Muse toggle** is enabled and the request comes from a US-eligible IP; applies via settings.yaml hot-reload WITHOUT restarting DSH; config is unversioned (harness home); owner to confirm on the web Models page.
