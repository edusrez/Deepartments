...
- **2026-08-20** — **Muse rollout rollback**: the `muse-spark-1.2-contributor` rollout to department heads (cordis.patch.yml) and delegation tools (dev preset) was REVERTED because it was broken at runtime: the harness maps thinking level `off` to the wire value `none`, and Console Go rejects it — `invalid_request_error: "reasoning.effort" does not support "none" with this model` — so every spawned muse agent 400'd before generating a single token. The 08-19 "enabled and WORKING" claim left no retained smoke evidence, and the 08-20 bulk rollout skipped the tier-4 headless smoke. Reverted: department heads to `deepseek-v4-flash` on `opencode-go` (commit `e38f339`); delegation rows (`tool-subagent`/`tool-subagent-fork`) to `deepseek-v4-flash` in the dev preset; the `dsh-llm-pi-ai` `reasoningSummary: "detailed"` hardcode removed (pristine upstream). The muse route stays DECLARED in dev `settings.yaml` (optional, off-by-default) with its `off:` mapping fixed to omit the reasoning param so the model is usable only if explicitly selected — note it is the Contributor tier (data trains Meta models; not for sensitive work). Reviewer PASS: `.dsh/reports/reviewer/2026-08-20-revert-muse-batch.md`.
