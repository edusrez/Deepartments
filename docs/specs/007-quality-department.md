...
key_findings:
  ...
  - OWNER AMENDMENT (2026-08-25, M4 docs-only): the QH is the CONSOLIDATOR + ANALYZER (verdict per round "todo bien" / "issue X → dirigido a Y (Z)"; findings to the Asistente AND/OR a PROGRAMMING REQUEST → IPD, a RESEARCH REQUEST → RD, or the Asistente for an owner decision). **LOTE A (2026-08-27): head/worker sleep is RETIRED** — the SLEEP DIRECTIVE mechanism is gone (heads stay idle|running); the QH's own sleep exception (D-Q7) is obsolete with it.
  - The worker-archive dice (D-Q2) and the host-rotation 100% mandate (D-Q3) are the same event hook: `qualityInspectDecision(kind, deps)` — a PURE injectable-rng function where kind 'host' ALWAYS true, and kind 'worker' rolls `rng() < (workerInspectProbability ?? 0.25)`. The kind 'head' branch (the retired head sleep) remains in code as R6 dead/residual logic — no head-slept event is ever emitted.
  - The remaining archive events have an EXISTING seam to hang the hook on (verified): worker retire dept_worker_retire → retirePost (invoke.ts:7593) + archiveWorkerSession (invoke.ts:7644); host dept_sleep host-branch → runHostRotation (invoke.ts:9479, S2.5 archives the OLD host session). (The head dept_sleep branch — archivePostSessionOnSleep — is RETIRED with the head sleep system, LOTE A.)
...
(§3.5/§7.2 — the QH is the CONSOLIDATOR + ANALYZER, verdict per round).
**LOTE A (2026-08-27, owner decision):** the head/worker sleep system is RETIRED —
heads and workers stay `idle|running`; the SLEEP DIRECTIVE mechanism (§7.2 — the
Asistente managing head sleep) is gone; the QD's anti-loop QH inspect exception
(D-Q7) is obsolete with the head sleeps (only the HOST rotation and the worker
retire dice remain).
...
1. **Every lifecycle archive is a QA sample.** Worker retires are sampled
   (25% by default, D-Q2) and host rotations are sampled 100%
   (D-Q3) — because a bad host archive is a *systemic* signal while a bad
   worker retire is a *noisy* one. (**LOTE A, 2026-08-27:** head sleeps are
   history — heads/workers no longer sleep, so the old head-archive sampling
   and the QH anti-loop exception (D-Q7) are gone.)
...
2. **G2 — The archive-event hooks (D-Q2, D-Q3).** In the org plugin, on the
   archive events — worker retire (`dept_post_retire`/`dept_worker_retire`) and
   host session rotation — emit an ADDRESSED QUALITY INSPECT
   directive to `quality-head` via the message bus. Worker retire: dice (default
   0.25). Host rotation: always (100%). (LOTE A: the head `dept_sleep` event is
   RETIRED with the head/worker sleep system.) The hooks NEVER throw, NEVER spawn a QD
   worker, and are non-fatal to the retire/rotation they hook.
...
- The **host 100% mandate is NOT a knob** — it is structural (D-Q3): the
  gate returns `true` for `kind 'host'` regardless of any probability.
  (LOTE A, 2026-08-27: the head branch — with the old NON-QH-head 100% &
  QH-own-sleep D-Q7 dice — is RETIRED with the head/worker sleep system; the
  pure function keeps the `'head'` branch as dead code, R6.)
- **Determinism for tests AND the single documented runtime override (D-Q2,
  ...
  default `0.25`) on the **worker** probability path. The structural host
  mandate is never
...
functions: `retirePost` and `runHostRotation`; the head `dept_sleep` branch is
RETIRED with the head/worker sleep system, LOTE A 2026-08-27).
...
- The hooks **NEVER throw** — wrap the whole emit in its own try/catch that
  degrades to `ctx.logger.warn`; the retire/rotation they hook still
  completes (the archive mark / rotation mark is the durable part).
- The hooks are **non-fatal** and **side-observable only**; a failed directive
  delivery never aborts the retire/rotation.
- The hooks emit at the SAME place the archive/rotation seal happens, so the
...
qualityInspectDecision(kind, deps): boolean
  ...
  deps: {
    ...
    headPostId?: string             // RETIRED (LOTE A 2026-08-27): the D-Q7 head-exception param — no head-slept events are ever emitted
  }
```

| kind | result |
|---|---|
| `'head'` | **RETIRED (LOTE A 2026-08-27)** — the head archive branch is dead code (R6): heads/workers no longer sleep, so kind 'head' is never resolved at the emit site. Kept in the pure function for history (R6), structurally true when called. |
...
- The host branch is **structural** — no probability, no knob, no env override
  can make it false (D-Q3 "never gated by the dice"); the retired head branch
  (the old D-Q7 head-exception) no longer has a live emit site.
- The **env override** `DEEPARTMENTS_QUALITY_INSPECT` (a number string in `[0,1]`)
  ...
  path — in tests and production alike,
  and it never turns the structural host mandate off.
...
(spec 005 §5.3). (LOTE A, 2026-08-27: heads no longer carry `dept_sleep` in their
own layer — only the host sleeps.) The messaging ACL (`aclDenyGround`) is generic
— a new `quality`
...
`dept_worker_retire` (the department-scoped retire tool) OR the legacy
`dept_post_retire` (host tool) — the hook:
...
3. on `true`, emits the ADDRESSED QUALITY INSPECT directive to `quality-head`
   with the worker's `postId`/`sessionId`/`archived` marker — the directive text
   carries the **explicit ANALYZE mission** (LOTE B, 2026-08-27): *"ANALYZE the
   retired agent: its log/session, the tools it used, its flows, its failures,
...
`dept_worker_retire` tool, the host `dept_post_retire`, the AUTO-RETIRE on
delivery to the manager head, and the boot-reconcile half-slept reap), so a
worker retired by ANY of them rolls the 25% sample. The idempotent no-op
(`entry.retired === true` early-return inside `retirePost`) returns BEFORE the
dice ⇒ a re-retire of an already-retired worker NEVER re-emits (R1, one
directive per REAL archive). The dice block is wrapped in a non-fatal
try/catch — a directive failure only warns, the retire path NEVER breaks.
...
### 6.2 Host session rotation — MANDATE 100% (D-Q3, host = "H")

(§6.2 "Head `dept_sleep` — MANDATE 100% (D-Q3, with the anti-loop D-Q7
exception)" is **RETIRED** — LOTE A, 2026-08-27: the head/worker sleep system is
gone, so there is no head archive event to inspect and no QH anti-loop exception.
The old section is kept as history only; the runtime `sleepMember`/`head-slept`
directive code stays as dead code (R6).)

The host branch of `dept_sleep` (invoke.ts:9397 near) performs the **host session
...
**QH lifecycle (LOTE A, 2026-08-27 — permanent idle|running).** The QH's round
ends with it producing the report/verdict to the Asistente; it then simply ENDS
its turn and stays idle until the next addressed message arrives (BOOT-QUIET).
Head/worker sleep is RETIRED: there is no SLEEP DIRECTIVE anymore (the Asistente
no longer manages head sleep — that mechanism is gone), and the QH never
concludes with `dept_sleep` (only the Asistente/host conserves its own session
rotation, spec 002). Memory is persisted with `dept_memo_write`. With no head
sleeps there is no D-Q7 anti-loop exception to document.
...
| Phase | Scope | Files |
|---|---|---|
| **Q1 — config + probability gate** | `org` config gains the `quality` block (D-Q1 + the `quality: { workerInspectProbability?: number }` knob, §4.1); `QualityConfig` + schema (mirror `health`); `qualityInspectDecision(kind, deps)` PURE function (§5.2) + the env override `DEEPARTMENTS_QUALITY_INSPECT` | `src/org.ts` (`Config` + schema), `src/invoke.ts` (the gate fn + env override), `test/invoke.test.js` (probability-gate unit tests: 0.25 / clamp / mandate 100% host (the retired head branch is R6 dead code, no live emit site) / deterministic seed+env) |
| **Q2 — archive-event hooks** | The archive-event hooks (D-Q2/D-Q3): worker retire dice (invoke.ts:7175/7593/7644 seam) and host rotation 100% (invoke.ts:9479 seam; the head `dept_sleep` hook is RETIRED — LOTE A 2026-08-27); the emit-to-`quality-head` directive seam (store.append + busDeliverToPost, daemon-not-a-catalog-member pattern); NEVER throw / NEVER spawn; non-fatal | `src/invoke.ts` (the hook seams + the resolve-quality-head + directive emit), `test/invoke.test.js` (hook wiring: worker dice OFF/ON, host always, non-fatal, no worker spawn) |
...
4. Headless smoke in `deepartments-dev-headless`: boot → `dept_who` shows the QH
   ...
   a worker retire rolls the dice (sampled under a fixed seed) → a host
...
1. **The hook must never break what it hooks.** The retire/rotation seams are
   non-throwing by contract (spec 005 §3.4, spec 006 §11); the QD directive emit
   must be wrapped so a failing emit degrades to a warn and the archive/rotation
...
2. **The dice must be exact and testable.** `qualityInspectDecision` is PURE
   ...
   host mandate MUST be structural (a bug turning it into a die would violate
   D-Q3). (The old head branch + the QH anti-loop die are RETIRED with the head
   sleep system — LOTE A 2026-08-27; the pure function keeps the branch as dead
   code (R6), never a live emit site.)
...
1. **The exact probability-gate signature / seed-injection seam (❓).** The PURE
   ...
   **structural host** mandate off; the retired head branch (D-Q7) has no live emit
...
- The exact seam-map file:line anchors for the archive events (seam report
  cross-check — currently verified in-repo: `retirePost` invoke.ts:7593,
  `archiveWorkerSession` invoke.ts:7644, host rotation invoke.ts:9479; the head
  `dept_sleep` archive was RETIRED with the head/worker sleep system — LOTE A).
...
- Whether the directive emit belongs in `retirePost` / `runHostRotation`
  directly or in a small `maybeEmitQualityInspectDirective(kind,
  entry)` helper invoked by each (recommended: a single helper, two call sites —
  no per-surface drift; the head-sleep call site is RETIRED with the head sleep
  system, LOTE A).
...
- **Invariants kept**: QD workers are ROOT agents (`worker-<slug>-<uuid>`, no
  ...
  hook only sends a bus directive — D2 principle); the host rotation mandate is
  structural (100%, D-Q3 — a bug must never make it a die); the head archive
  mandate (with the D-Q7 anti-loop exception) is RETIRED with the head/worker
  sleep system (LOTE A 2026-08-27 — the pure function keeps the 'head' branch as
  dead code, R6, with no live emit site).
- **Surprises verified**: (1) the QD is the ONLY department where a worker's report
  ...
  `reports/` (D-Q6) — a deliberate divergence; (2) the archive events ALL
  have an EXISTING seam to hang the hook on — no new archive machinery is needed
  (`retirePost` / `archiveWorkerSession` /
  `runHostRotation`; the head `archivePostSessionOnSleep` seam is RETIRED with
  the head sleep system, LOTE A); (3) the hooks emit via the **daemon-not-a-catalog-member**
...
- **Primary source refs for builders**: docs/specs/005-internal-programming-
  ...
  archiveWorkerSession 7644, host rotation 9479, runJobForDepartment 6055,
  spawnWorkerForDepartment 6130,
  busDeliverToPost 8415, busDeliverToHost 8479; the head dept_sleep branch
  archivePostSessionOnSleep 7667 is RETIRED with the head/worker sleep system,
  LOTE A), presets/ (role-template
